...
public:
	...
	void deleteSmall();
	//Funtion to delete one instance of an item

	void deleteAll(int item);
	//Function to delete all instances of an item
...
void unorderedLinkedList<Type>::deleteSmall()
{
	...
	else
	{
		...
		while (current != NULL)
		{
			if (current->info < low->info)
			{
				low = current;
				trailLow = trailCurrent;
			}
		...
		trailLow->link = low->link;
		delete low;
	}
}

template <class Type>
void unorderedLinkedList<Type>::deleteAll(int item)
{
	nodeType<Type> *current; 
	nodeType<Type> *trailCurrent; 
	nodeType<Type> *match;
	nodeType<Type> *trailMatch;
	...
	{  
		...
		while (current != NULL)
		{
			if (current->info == item && current == this->first)
			{
				match = this->first;
			...
			{
				match = current;
				trailMatch = trailCurrent;
				trailMatch->link = match->link;
				current = current->link;
				delete match;
			}
...
void unorderedLinkedList<Type>::insertFirst(const Type& newItem)
{
	...
	if (this->last == NULL)     //if the list was empty, newNode is also
		this->last = newNode;   //the last node in the list        
}//end insertFirst
